...
new Requestも、APIとしての使用が決まっています。ただこのapiはランタイムのapiなので、ecma262ではなくwhatwgのfetchの仕様書に記述されています。https://fetch.spec.whatwg.org/#dom-request

書き方自体はecma262の仕様書とほとんど同じで、あとはランタイム特有の説明だったりそもそもの見方だったりに慣れればなんとなく理解できてきます。

そもそもこのapiは、RequestInfoとRequestInitという二つの引数を取ります。仕様書のWebIDL的には以下のよう書かれます。
...
RequestInitは渡せるプロパティの数がかなり多いので、詳しくは[mdn](https://developer.mozilla.org/ja/docs/Web/API/RequestInit)やnode.jsの各ランタイムのd.tsなどを見ると良いでしょう。
...
inputがstringだった場合、urlになるようにparseして、失敗すれTypeErrorになるようになっています。
...
node.jsはブラウザと互換性かなり高くて、ほぼ全てのRequestInitを適切に処理しています。

andromedaで実装した際も、node.jsはかなり参考になりました。

これに対してDenoは少し面白くて、こちらはステップ13から21をスキップしています。
...
これが原因でissueも立ってはいるので、なんでスキップしているの？と聞いてみたところ、referrerを扱わないから意図的に実装していない

https://discord.com/channels/684898665143206084/1387791633814257704

とのことでした。

まぁこれは、minumum common apiにも書かれていて、
