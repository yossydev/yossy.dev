---
title: "AndromedaのRust to JS実装を見る"
...
`pub fn new_extension() -> Extension {` の時の、`Extension` という struct が少し怪しい。
その実装は以下に存在している。
...
サードパーティに頼っているのかと思いきや（oxc には頼っているけど）自前の、というか ECMA-262 に記載のあるメソッドを使用しているとは思わなかった。
